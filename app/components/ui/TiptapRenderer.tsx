...
const renderMarks = (text: React.ReactNode, marks?: TiptapMark[]): React.ReactNode => {
  ...
  return marks.reduce<React.ReactNode>((acc, mark, idx) => {
    switch (mark?.type) {
      ...
      case 'highlight':
        return (
          <mark key={idx} className="rounded-sm bg-amber-100 px-0.5 text-slate-800">
            {acc}
          </mark>
        );
      case 'textStyle': {
        const bg = mark.attrs?.backgroundColor as string | undefined;
        const color = mark.attrs?.color as string | undefined;
        if (bg || color) {
          return (
            <span key={idx} className="text-slate-700" style={{ backgroundColor: bg ? 'transparent' : undefined }}>
              {acc}
            </span>
          );
        }
        return <span key={idx}>{acc}</span>;
      }
      default:
        return acc;
    }
